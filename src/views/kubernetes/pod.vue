<template>
  <div>
  <el-drawer :with-header="false" title="Pod" size="80%" :visible.sync="open" :destroy-on-close="true" append-to-body @close="cancel">
    <el-card class="box-card">
      <el-form ref="queryForm" :model="queryParams" :inline="true" label-position="left">
        <el-form-item label="命名空间" prop="roleName">
          <el-select v-model="namespace" clearable filterable placeholder="请选择命名空间">
            <el-option
...
<!--          />-->
        ...
        <el-form-item label="名称" prop="podName">
          <el-input
            v-model="queryParams.ClusterPodDTO.name"
            placeholder="请输入名称"
            clearable
            size="small"
            @keyup.enter.native="handleQuery"
...
<!--          </el-button>-->
        ...
        <el-table height="600" type="mini" v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
          ...
          <el-table-column
            ...
            :show-overflow-tooltip="true"
          />
          <el-table-column fixed="right" label="操作" min-width="200" align="center" class-name="small-padding fixed-width">
            <template slot-scope="scope">
...
<script>
import {namespaceList, podList} from "@/api/kubernetes/cluster";
...
export default {
  ...
  methods: {
    ...
    getPodList() {//获取命名空间
      this.loading = true
      podList({'id': this.clusterId,'namespace':this.namespace}).then(response => {
        if (response.code === 2000) {
    ...
    getList(name) {
      ...
      podList({'name': name}).then(response => {
        if (response.code === 2000) {
