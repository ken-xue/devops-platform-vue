        ...
        <el-col :span="24">
          <el-form-item label="JDK版本" prop="jdk">
            <el-select v-model="form.jdk" placeholder="请选择JDK版本" clearable :style="{width: '100%'}">
              <el-option  v-for="(item, index) in JDKVersions" :key="index" :label="item.label" :value="item.value" :disabled="item.disabled"></el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="24">
          <el-form-item label="Maven版本" prop="mvn">
            <el-select v-model="form.mvn" placeholder="请选择Maven版本" clearable :style="{width: '100%'}">
              <el-option v-for="(item, index) in mavenVersions" :key="index" :label="item.label" :value="item.value" :disabled="item.disabled"></el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="24">
          <el-form-item label="构建命令" prop="buildCMD">
...
export default {
  ...
  data() {
    return{
      initScript: '# maven build default command\n' +
      ...
      rules: {
        ...
        buildCMD: [{required: true, message: '构建命令不能为空', trigger: 'blur'}],
